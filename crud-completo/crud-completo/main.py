...
def atualizar_produto(produto_id: int, dados: ProdutoPatch):
    produto = encontrar_produto(produto_id)
    if dados.nome is not None: produto = produto['nome'] = dados.nome
    if dados.preco is not None: produto = produto['preco'] = dados.preco
    if dados.estoque is not None: produto = produto['estoque'] = dados.estoque
    return produto
